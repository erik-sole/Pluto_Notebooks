...
# ╔═╡ 03ec51eb-bc9d-4b58-b7b3-4710994ace24
begin 
	using PlutoUI
	using Makie
	using Interact
...
# ╔═╡ b7d53dde-b7a2-498a-b6d1-970f81ec71b9
md"# Diamond Model Dynamics (Canonical)"

# ╔═╡ 98d441d0-92e8-11ef-0bde-9fe805825cc1
md"In this notebook we will be able to simulate the dynamics of a OLG Diamond Model in canonical form, that is, with a Cobb-Douglas production function and log utility, without bubbles or government debt. Later on the notebook, we will explore different  shocks."

# ╔═╡ 014fccb0-7b02-487d-b4ec-9d6db474ff73
md"### Parameters"

# ╔═╡ f50b4e1e-167f-4c3a-a1dd-f3a2628047be
md"Here you can use the sliders to change parameters and see how the law of motion changes!"

# ╔═╡ d23d6804-432c-4ce0-865a-9c8d9164a4e4
begin
	θ = @bind  theta PlutoUI.Slider(0:0.5:15)
	γ =  @bind  gamma PlutoUI.Slider(1:0.1:5)
	λ =  @bind  lambda PlutoUI.Slider(1:0.1:5)
	α =  @bind  alpha PlutoUI.Slider(0:0.1:0.99)
	β =  @bind  beta PlutoUI.Slider(0:0.1:5)
md"Theta (θ) : $(θ) \
Effective Labor Growth (γ): $(γ) \
Population Growth (λ): $(λ) \
Discount Factor (β): $(β) \
Share of capital (α): $(α)"
end	

# ╔═╡ 5b1f5b48-74ef-43d9-8720-e102c68c51c1
md"### θ = $theta, γ = $gamma, λ = $lambda, α = $alpha, β = $beta"

# ╔═╡ 6fe00e45-4c05-4782-bfcc-8d948721c5b8
begin
	
# k_ss = ((1/(γ*λ))*(β/(1+β))*θ*(1-α))^(1/1-α);
	k_ss = ((1/(lambda*gamma))*(beta/(1+beta))*theta*(1-alpha))^(1/1-alpha);
	    k_ss_formatted = @sprintf("%.3f", k_ss);

md"### $k^* = (\frac{1}{\lambda\gamma}\frac{\beta}{1+\beta}\theta(1-\alpha))^\frac{1}{1-\alpha}$ = $(k_ss_formatted)"

end

# ╔═╡ 35552639-3968-4efc-beea-32407f84e977
begin
k = collect(0:exp(-6):2.5*k_ss)
dynamics = function(gamma,lambda,beta,alpha,theta,k)
	result = 1/(gamma*lambda)*(beta/(1+beta))*(1-alpha)*theta*k.^(alpha)
end
results = dynamics(gamma,lambda,beta,alpha,theta,k)

f = Figure()	
ax = Axis(f[1, 1], xlabel=L"k_t", ylabel=L"k_{t+1}", title="Capital Dynamics")
xlims!(ax,0,1.5)
ylims!(ax,0,1.5)
lines!(ax, k, results, color=:red, label = "Law of motion")  
lines!(ax,k,k, linestyle =:dot , color=:black, label = "Diagonal Line")
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "123f4fda0309ba13e65ff11c0bfb99994b51ecb8"
